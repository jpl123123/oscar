...
**格级对比图：同一个 K 格（393,216B）在原生 bf16 与 packed×2 下的划分**（数字经三重审计：
format.py 常量 + store_kernel 实际写入偏移 + 独立复算/真机日志对账，见下表）：

```text
原生 bf16（槽宽 512B）：一格装 768 个 token
字节 0          512          1,024  …                        393,216
    ┌─────────────┬─────────────┬─────┬─────────────────────┬─────────────┐
    │ token 0     │ token 1     │ …   │                     │ token 767   │
    │ 512B 满排    │ 512B        │     │                     │ 512B        │
    │(256个bf16原值)│             │     │                     │             │
    └─────────────┴─────────────┴─────┴─────────────────────┴─────────────┘
    768 × 512B = 393,216B ✓                每 token·head K+V = 1,024B

packed×2 int8（槽宽 256B）：同一格装 1,536 个 token
字节 0       256       512   …                                        393,216
    ┌────────┬────────┬─────┬────────────────────────────────────┬────────┐
    │token 0 │token 1 │ …   │                                    │token1535│
    │96B载荷  │96B载荷  │     │                                    │96B载荷  │
    │+160B闲 │+160B闲 │     │                                    │+160B闲 │
    └────────┴────────┴─────┴────────────────────────────────────┴────────┘
    1,536 × 256B = 393,216B ✓（格子一字未变！）   每 token·head K+V = 512B
    OSCAR 实际写 K 格 = 1,536×96 = 147,456B（37.5%）；V 格 = 1,536×64 = 98,304B（25%）
    （"96B 载荷"是跨度：8B 元数据+24B 垫+64B 编号，槽内放大见 §2.5.9）
```

| 图中数字 | 代码/日志出处 | 审计结果 |
|---|---|---|
| K 载荷 96B（8+24+64）、V 载荷 64B、逻辑槽 160B | format.py:19-33（K_IDX_OFF=32→32+64=96；V_IDX_OFF=96−96=64） | ✓ |
| K 槽写 [+0..+8) meta（含 V 的 scale/zero @+4..+8）、[+32..+96) idx；V 槽写 [+0..+64) | store_kernel.py:82-95（index_put_ 逐偏移，与图逐字节一致） | ✓ |
| 原生槽 512B / packed 槽 256B（= D×dtype 字节） | backend.py:164-172 几何对账断言 slot∈{256,512}；真机日志"★ 几何对账: K 槽 256B/V 槽 256B → packed×2" | ✓ |
| 每格 768/1,536 token、格宽 393,216B 两算同值 | 768×512=1,536×256=393,216（独立复算 ✓）；真机影子池 shape 13,380=nb×12 → block_chunk=12 → block=1,536 | ✓ |
| 写入率 37.5%/25%/31.25% | 147,456/393,216、98,304/393,216、160/512（复算 ✓） | ✓ |
