...
区B  K/ssm 共区 [21,027,840, 559,340,544)  每格 393,216B
  ...
  同一片区B（538,312,704B）的两种视图——两把尺子量同一堆字节，数值对账：

  |              | K 视图（FULL 的尺子）              | ssm 视图（GDN 的尺子）           |
  |--------------|-----------------------------------|----------------------------------|
  | shape（容器） | (8,214 = **1,369×6**, 128, 1, 256) | (**1,369**, 12, 128, 128)         |
  |              | bf16                              | bf16                             |
  | 每格字节      | 128×1×256×2B = **65,536B**         | 12×128×128×2B = **393,216B**      |
  | 容器总账      | 8,214 × 65,536B = **538,312,704B** | 1,369 × 393,216B = **538,312,704B** ✓ 相等 |
  | 本请求占用    | 43 格 × 6 = 258 逻辑块             | 3 租户层 × 5 格 = 15 格            |
  |              | 258 × 65,536B = **16.9MB**         | 15 × 393,216B = **5.9MB**         |

  换算锚点：1 个 ssm 格 = 6 个 K 逻辑块（393,216 = 6 × 65,536）——对齐等式
  512×768==393,216 的直接体现；两容器总账相等 = 两视图共字节的证明。
