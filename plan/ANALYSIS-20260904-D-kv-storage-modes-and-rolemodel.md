...
区A  conv 尺（GDN 专用）字节 [0, 21,027,840)，每格 15,360B
  块号 →   0 ………… 42          43 ………… 57           58 ………… 1368
        ┌───────────────┬──────────────────┬────────────────┐
        │  死 ×43 格      │ GDN conv ×15 格   │    空 ×1,311 格  │
        └───────────────┴──────────────────┴────────────────┘
          ↑FULL 块的 conv 格没人写    ↑3 个 GDN 层各自的卷积抽头

区B  K/ssm 双视图共区 字节 [21,027,840, 559,340,544)，每格 393,216B
  块号 →   0 …………………… 42（FULL 领）           43 …… 57（GDN 领）    58 … 1368
        ┌───────────────────────────────┬──────────────────┬───────────────┐
        │  K 视图把每格再切 6 份：          │  ssm 视图整格=1 份 │     空 ×1,311   │
        │  块0→K[0..5]   块1→K[6..11]      │  状态（不细分）     │      格        │
        │  ……  块42→K[252..257]           │  ×15 格(记账/实12) │               │
        └───────────────────────────────┴──────────────────┴───────────────┘
  两种读法（数字 ↔ 上图格子一一对应）：
    K   视图 (8,214 = 1,369×6, 128, 1, 256) bf16 → 本请求 = 43 格 × 6 = 258 逻辑块
                                                    × 65,536B/块 = 16.9MB
    ssm 视图 (1,369, 12, 128, 128) bf16            → 本请求 = 15 格 × 393,216B = 5.9MB
                                                    （驻留实块 3组×4=12，余为 null 占位）

区C  V 尺（FULL 专用）字节 [559,340,544, 1,097,653,248)，每格再切 6 逻辑块（与 K 同构）
  块号 →   0 ………… 42                43 ………… 57       58 ………… 1368
        ┌───────────────────────┬───────────────┬────────────────┐
...
本池占用合计 = 43×(65,536×6×2 K+V) + 15×(15,360+393,216) = 33.8MB + 6.1MB ≈ 39.9MB（记账 58/1,369 = 4.24%）
