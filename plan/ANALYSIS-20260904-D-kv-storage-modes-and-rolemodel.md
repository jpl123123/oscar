...
**分组**：64 层若每层独享一个池就太浪费，于是按同型分桶、交错分组（[V]
kv_cache_utils.py:1142-1195，FIXME :1146-1157 讲的就是避免逐组 padding），得到 4 组
各 16 层——组数取桶宽最小值 16，**16 个池**由此而来；第 i 池住 [GDN组0第i层,
组1第i层, 组2第i层, FULL第i层]。每个池就是一根一维张量 `torch.zeros(P×nb,
dtype=int8)`（[VA] model_runner_v1.py:4124）——选 int8 是因为每元素恰 1 字节，池因此
...
三个区的宽度不是拍脑袋定的——每一格都能从模型配置逐步算出。**每格字节的算法**：
...
**格子归谁用？——块号**：引擎的分配单位是"块号"（`KVCacheBlock.block_id`，
[V] kv_cache_utils.py:117-149），生命周期由全局唯一的 `BlockPool` 管理，代码词汇
（block_pool.py）：**分配** = `get_new_blocks(n)`（:333-360，`free_block_queue.
popleft_n(n)` 取出 + `block.ref_cnt += 1`）；**释放** = `free_blocks()`（:419-436，
`ref_cnt -= 1`，归零者回到 `free_block_queue`）。分配挂在各 group 自己的
`SingleTypeKVCacheManager.req_to_blocks[request_id]` 上（:276-289）。一个块号 b
在三个区各占一格，当前归属（在谁的 req_to_blocks 里）决定格子的身份：
- 归 FULL 组 → 区B 格当 K（=6 逻辑块）+ 区C 格当 V；**区A 格无人写入**（~1.9%/块，
  官方注释名 "(kv_padding)"）；
- 归 GDN 组 → 区A 装 conv + 区B 装 ssm；**区C 格无人写入**（~49%/块，
  "(mamba_padding)"）。
  归属随 get_new_blocks/free_blocks 流动（释放后改派另一组，原死格即可被写入）。

**页 = 块号的价格**：既然一个块号在三区各占一格，它的全账成本就是三格相加 =
15,360 + 393,216×2 = **801,792B = 页宽**（[V] kv_cache_interface.py:830 "size in
bytes"）。所以"页"只是记账单位，不是字节里一段连续区间——这正是 §2.5 总图里三区
互不相邻、"页"却依然成立的原因。又因一个块号要在 16 个池同步占格，全卡单价 =
页宽×16（memory_per_block，kv_cache_utils.py:911）——这个单价直接引出 §2.3 的 nb。
...
→ 交集被强制为 [15,360nb, 408,576nb) = 恰好 K 半区。至于为什么叠在 K 而不是 V——
纯属切片顺序：若 FULL 内部改排 [V|K]，ssm 就会叠在 V 上，功能完全等价。
（前提澄清，注意两个层次：**切视图**层——conv 段 [0, 21,027,840) 与 ssm 段
[21,027,840, 559,340,544) 首尾相接，并集是张量的连续前缀；"从 0 顺序切"指此。
它不表示某块号的 conv/ssm 相邻（43 号块 conv 在字节 660,480、ssm 在 37,936,128），
也不表示 GDN 占用从 0 号格开始——**占格子**层由分配决定：块号 0–42 在 FULL 组的
`req_to_blocks` 里（`get_new_blocks` 取出、ref_cnt≥1）、43–57 在 GDN 组的
`req_to_blocks` 里、**58–1368 仍在 `BlockPool.free_block_queue` 中（ref_cnt==0，
`num_free_blocks` 的一部分）**——三区的 58–1368 格同为初始零，因为写入方只对已分配
的块号执行；这与切法无关。"混"的
只有 ssm≡K。）
再往上还有一层元理由——**为什么非要单一池**：① 历史路径：vLLM v1 的 BlockPool/
num_blocks/前缀 hash/调度比较全是单一命名空间建的，hybrid 是后塞进去的，"统一页宽
+ pad"就是适配层（TQ #39931 在 CUDA 同款）；② 弹性：单一池的页可在 FULL/GDN 间
流动，"每块最多浪费 49%（GDN 块）"是**任何负载形态的上界**；静态分区（GDN 一区、
K/V 各自干净区）必须猜比例，负载形态一变搁浅的显存可远超饱和 21% 死字节的水平。
别名 = 单一池约束下四个连续 span 求极值的最优解：不别名必贵一个 393,216 span。）
...
表里四个量由四条等式连成一体：区B 格宽 = block_size×Hk×D×dtype字节 = 393,216
（两种几何都必须成立——这行就是对齐等式本身）；往下依次推出 每格逻辑块数 =
block_size÷128、请求 FULL 块数 = cdiv(seq, block_size)、页宽 = block_size×K+V 每
...
nb 由预算单方面决定，与请求大小无关——请求只决定 `get_new_blocks` 取走几个块号
（§2.4）。也正因为
它对预算敏感，真机曾实测 nb=1,115：当时插件路径的肥大临时峰值被计入"激活峰值"，KV
预算被吃掉 ~2GB；prefill 分块修复（a874b5d）削掉峰值后，预算回升、nb 应回 1,300+。
...
```text
═══ 统一主轴 = block_id（0…1,368）═══  本请求：FULL 组 req_to_blocks=[0..42]、
   GDN 组 req_to_blocks=[43..57]（示意：取自 free_block_queue 的连续号）
...
区B  K/ssm 共区 [21,027,840, 559,340,544)  每格 393,216B
  块号 →   0 …………………… 42（FULL 领）      43…47   48…52   53…57（GDN 领）  58 … 1368
        ...
        └───────────────────────────────┴────────┴────────┴────────┴──────────────┘
  每层 5 格 = 1 提交态 + 3 投机态 + 1 过渡缓冲；稳态实块 4。为什么一层要 5 格？
  ——FULL 的 KV 是追加式（拒绝=不读那几行，回滚≈0），GDN 的状态是破坏性更新
  （每 token 原地改写 S），MTP 投机前必须留快照：3 个 draft 有 4 种拒绝结局
  （拒在第 1/2/3 个或全收），对应 4 个回滚点（提交态 + 3 个 draft 前推态），
  第 5 格是换块瞬间的双缓冲余量（记账 2+spec 公式）。无 MTP 则一层只需 1 格。
  每格同时带区A 同块号的 conv 快照（卷积窗同样随 token 前移）
  ——区B 的占用讲完了。但区B 之所以叫"共区"，是因为 FULL 与 GDN 对同一片字节
  （538,312,704B）持有两套对称的读法；把两把尺子按"块号格"统一表达，行行对账：
  ...
  ¹ 表里的 shape 按"块号格"分组书写；实现张量把前两维展平成 (1,369×6, 128, 1, 256)
  = (8,214, 128, 1, 256)。展平后第 L 个逻辑块属于哪个块号？L÷6 得块号、L%6 得格内
  第几个——所以寻址公式才写成 L = 块号×6 + i（block_table.py:288-303 同源）。
...
**图里 ssm 格装的东西值得单独定义**。ssm = DeltaNet 的递归状态矩阵：每个注意力 head
维护一份 d_v×d_k 的矩阵 S——线性注意力的全部历史**压缩进这一个固定矩阵**（不随序列
增长；这也是 chunked prefill 必须按块对齐的原因：状态只能在整块边界 checkpoint）。
模型共 48 个 value head、TP4 切成每卡 12 个、head_dim=128，于是**一层一份**状态：

  (12, 128, 128) = 12 × 128 × 128 = 196,608 个元素 × 2B(bf16) = **393,216B**
  （12 = num_v_heads 48 ÷ TP4；[V] mamba_utils.py:228-232
   `temporal_state_shape=(num_v/tp, head_v, head_k)`；头数分解为[推断]）

**393,216 与"3 层"没有任何乘法关系**——因式分解 12×128×128×2 里没有因子 3，它是
"一层一份"的尺寸；3 层改变的是格子**数量**而非格宽（每池 GDN 占用 = 3 层 × 5 格 ×
393,216B，即 §2.4 读数表"3 租户层 × 5 格"的含义）。两个容易混淆的"3"顺便分开：
conv 形状 (2,560, **3**) 里的 3 是卷积窗（conv_kernel−1），也与层数无关。
还有一个隐藏前提值得点破：393,216 = 6 × 65,536，恰好被 128×512 整除——原生几何
才能解出 block_size = 128×6 = 768；若模型头数配置破坏这个整除性，`attn_block_size
= 128 × cdiv(...)` 会向上取整、assert 512×block==393,216 失败，vllm-ascend 直接
拒绝启动（对齐等式不是恒成立的，是这个模型的头数配置恰好让它可解）。
...
③ 单一块号空间（调度/前缀缓存/MTP lookahead 统一）。

**代价的完整账本（不报喜藏忧版）**——"零 padding"只对**字节衔接**成立（三区首尾
相接无间隙）；**记账层面系统从头到尾在 padding**（`page_size_padded` 机制：MambaSpec
真实 408,576B 记成 801,792B，上游日志原文 "Padding mamba page size by ~49%"）：

| # | 忧 | 量化 |
|---|---|---|
| 1 | **饱和负载 ~21% 池字节结构性死置**（FULL 59.5%×98.1% + GDN 40.5%×51.0% ≈ 79% 利用） | 最大的一项，来源=每 GDN 块的区C 死格 |
| 2 | 区A 常年低占用 | 20MB/池全额预留，稳态仅 15 格×并发被写（并发 37 封顶约 41%） |
| 3 | **mamba "all" 模式死格暴涨（隐藏雷）** | GDN 块变 cdiv(seq,bs)+3/组：32K 请求 ≈138 块×393,216B×16 池 ≈ **868MB/卡/请求**（align 模式仅 ~90MB） |
| 4 | packed 槽内闲置 | 160/512 = 31%（§4 四代阶梯） |
| 5 | 额外显存 | MTP 影子池 ~2.15GB/卡 + staging ~134MB/卡 |
| 6 | 前缀 hash 粒度 768/1,536 | 短前缀复用率下降（语义层） |

OSCAR 载荷视角再叠加：饱和态实际有效载荷 ≈ 39%（0.595×30.7% + 0.405×51.0%）。
根治方向同 §7：TQ 式"attention 页自定义 + mamba 页 pad 到 attention 页"，把 49%
的死区从"每块"降到"每页尾"的个位数。
...
上面每个偏移和数字都能对到具体代码行——逐项审计如下（数字 ↔ 代码/日志三重验证）：
...
int8_per_token_head` 把槽宽真改成 256B（视图 int8）⇒ 每个 block_id 的记账容量
（block_size 个 token）翻倍 ⇒
...
**packed×2 的配套：MTP 草稿层**。int8 池带来一个新问题——原生路径没有 per_token_head
算子（grep 零命中），而 MTP 草稿层恰恰必须 BF16（量化草稿会击穿接受率）。解法是 [W]
`mtp_shadow.py` 把 MTP 层 kv_cache 入参替换为页外 BF16 影子池：形状取自原生逻辑视图、
块号一一对应，原生 forward :1527 自行重绑定，写读因此 100% 走原生算子。代价 ≈2.15GiB/
rank（nb=1,369）懒分配——花的是 utilization 之外的 10% 余量（3.2GiB ≥ 2.15GiB ✓），
不侵占 KV 预算。

阶梯的下一档为什么直接跳到 PR 紧排（7.5×），没有中间的 128B（×4）？——
**int4 / 128B 可行性账**：
...
窗口是滑动的（尾窗随生成不断后移），仓库必须边装边淘汰——**环 + owner tag** 就是
零记账的淘汰机制：行号 = 逻辑块号 % 64，新块覆盖同号旧行（零记账淘汰）；tag 对得上
...
仓库建好了、政策定了，剩下的问题是：谁真的在读它？本部署三条路径状态各异——
