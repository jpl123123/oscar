...
**为什么 packed×2 是"真节省"、最初版是"假压缩"——记账机制一句话讲透**：
引擎只按**槽宽度**记账（一个块号装多少 token = 格宽 ÷ 每 token 字节；发块号、算
KV usage、定并发全按它）。最初版槽宽 512B（bf16 视图）没变、只是槽内放了个 64B——
**记账员不知道你省了**，仍按 512B/token 发牌 ⇒ **0 节省**（每 V 格闲置 448B，
KV usage/并发与原生一模一样——即本文件 §0 说的"约等于无效"）。packed×2 把槽宽
**真的**改成 256B（int8 视图）⇒ 记账员按 256B/token 发牌 ⇒ 同预算容量 ×2。
四代阶梯（一个 token·head 的 V 侧账）：

| 阶段 | V 槽宽 | 实际写入 | 闲置 padding | 密度 |
|---|---|---|---|---|
| 原生 bf16 | 512B | 512B 原值 | 0 | 1× |
| OSCAR 最初版（假压缩） | 512B | 64B 编号 | **448B 全浪费** | **1×（0 节省）** |
| **packed×2（本图 = 现状）** | **256B** | 64B 编号 | 192B | **2×（真节省）** |
| PR 紧凑布局（需源码 PR） | 无格子，68B 连排 | 68B | ~0 | **7.5×** |

（槽内字节级布局见 §2.5.9；剩余 192B 是给 group16 升级预留的余量：K 侧 64B meta +
64B idx = 128B、V 侧同，两格 256B 各自恰好装得下。）
...
#### 2.5.8 物理字节序证据链：torch.zeros 的真实排布 = 图的排布；排布本身零浪费

**源码证据链（6 条原文，全部亲读）**：

1. `torch.zeros(kv_cache_tensor.size, dtype=torch.int8)`——**一根一维连续张量**
   （[VA] model_runner_v1.py:4124；torch.zeros 产出 C-连续）。
2. FULL 分支三刀全是**切片**（零拷贝）：`:4571 attn_tensor_page_size = nb×393,216`；
   `:4574 conv_block_padding_size = raw.numel() − 2×attn_page = 15,360nb`；
   `:4575-4577 raw_kv = raw[15,360nb:]；k = raw_kv[:393,216nb]；v = raw_kv[393,216nb:]`；
   `:4637/:4642 .view(dtype).view(shape)`——连续切片的零拷贝重解释。
3. GDN 分支从 0 **顺序累进**：`:4705-4713 for shape in (conv, ssm): tensor =
   raw[start:target]...` → conv 占 [0, 15,360nb)，ssm 占 [15,360nb, 408,576nb)。
4. **稠密顺序是被 Ascend 约束选中的**——官方 NOTE 原文（:4688-4689）：
   *"NOTE(zxr): in order to keep all tensor contiguous, we align ssm and kv
   block with same page size"*——三区稠密 = 每个 view 都是连续切片；若按页交织
   [A|B|C]×nb，所有 view 都要 as_strided 跨页跳跃（非连续），aclnn 算子不可用/低效。
5. 我们自己的探针也在守护这件事：store 参考实现 `assert k8.is_contiguous()`
   （[W] store_kernel.py:63）——运行时实证视图确为连续切片。
6. 三区宽度对账：15,360nb + 393,216nb×2 = 801,792nb = P×nb（铺满、无排布级 padding）。

**可运行玩具证明（numpy 复刻三刀切片，nb=1,369 实算）**：

```text
conv 格 0     = 字节 [0, 15,360)          ← 张量最前面
conv 格 1368  = [21,012,480, 21,027,840)  ← 区A 末尾
K 逻辑块 0    = [21,027,840, 21,093,376)  ← 紧贴区A 之后（同基址=ssm 格 0 起点）
K 逻辑块 8213 = [559,275,008, 559,340,544)← 区B 末尾
V 逻辑块 8213 = [1,097,587,712, 1,097,653,248) ← 张量最末尾
铺满性：区A+区B+区C = 1,097,653,248 == P×nb ✓（三区互不重叠、恰好铺满）
别名：ssm[43]=[37,936,128, 38,329,344) == K[258..263] 六块连续区间 ✓（b=57/1368 同）
无缝：conv 格 b 的尾 == 格 b+1 的头，全区连续 ✓
```

**真实字节布局总图（所有偏移由上式实算，32K 请求着色）**：

```text
━━ 总览：一根 1,097,653,248B 连续张量的字节标尺 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━
0                    21,027,840               559,340,544               1,097,653,248
├──── 区A：conv ─────┼──── 区B：K ≡ ssm ──────┼──── 区C：V ─────────────────┤
│ 1,369 格 × 15,360B │ 1,369 格 × 393,216B    │ 8,214 逻辑块 × 65,536B      │
│ （GDN 专用，不细分） │ = 8,214 K 逻辑块×65,536 │（=1,369 格×393,216 再切 6）  │
└────────────────────┴────────────────────────┴────────────────────────────┘

━━ 放大窗① 张量头部：区A 的 conv 格（32K 请求着色）━━━━━━━━━━━━━━━━━━━━━━
字节 0          645,120  660,480              875,520 890,880              21,012,480
    ├─────────────┼────────┼─────────────────────┼──────┼────────────────────┼─────────┤
    │conv[0..42]死×43格│conv[43..57]活×15格│conv[58..1368]空×1,311格        │conv[1368]│
    │（FULL 块的conv格）│（3 个 GDN 层各 5 格） │                                │          │
    └─────────────┴────────┴─────────────────────┴──────┴────────────────────┴─────────┘
      conv[0]=[0,15,360)  conv[42]=[645,120,660,480)  conv[43]=[660,480,675,840)  每格 15,360B

━━ 放大窗② 区A|区B 交界 @ 字节 21,027,840 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ... conv[1367] │ conv[1368]=[21,012,480,21,027,840) │ K[0]=[21,027,840,21,093,376) │ K[1] ...
                                    ↑ 同一地址 21,027,840 = 区A 尾 = 区B 头 = ssm[0] 头

━━ 放大窗③ 区B 中段：同一字节的两种标签（K 段只有上行，GDN 段两行同标）━━━━
字节          37,542,912       37,936,128                38,329,344      43,441,152
    K  视角 → ├ K[252]…K[257] ┤├── K[258]…K[263] ──……──├ K[342]…K[347] ┤├ K[348] 起空…
    ssm 视角 → ├ ssm[42]（空格）┤├──── ssm[43] 组0·第i层 ──……── ssm[57] 组2尾格 ─┤
                    ↑请求最后 6 个 K 活块      ↑同一段字节 [37,936,128,38,329,344)：
                    （token 31,744–32,767）      上行读作 K[258..263]、下行读作组0 的 ssm 状态
    K 活段 [21,027,840, 37,936,128) = 258 块 = 请求全部 K；其后 15 个 ssm 格 = 请求全部 GDN 状态

━━ 放大窗④ 区B|区C 交界 @ 字节 559,340,544 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ... K[8212] │ K[8213]=[559,275,008,559,340,544) │ V[0]=[559,340,544,559,406,080) │ ...
                          ↑ ssm[1368]=[558,947,328,559,340,544) 尾 = K[8213] 尾 = V[0] 头

━━ 放大窗⑤ 区C：V 逻辑块（与 K 同构，无第二标签）━━━━━━━━━━━━━━━━━━━━━━
字节 559,340,544       576,248,832            582,147,072                 1,097,653,248
    ├ V[0..257]活×258块 ┤├ V[258..347]死×90块 ┤├ V[348..8213]空×7,866 块 ←V[8213] 到张量尾 ┤
    │=请求全部 V 16.9MB  ││=GDN 块 43–57 的 V 格 │                              │
    └────────────────────┴┴─────────────────────┴──────────────────────────────────────┘
      V[42]=[562,093,056,562,158,592)  V[258]=[576,248,832,576,314,368)  V[8213]=[1,097,587,712,1,097,653,248)
```

读图三规则：**① 一根张量、三个稠密区，地址 0→1,097,653,248 连续排满、无空洞**；
**② 区B 同一段字节两行标签——K 视角按 65,536B 切（上行）、ssm 视角按 393,216B 整格
读（下行），两行区间逐字节重合**；**③ 死/活只跟块号归属走**（窗① conv 前 43 格死、
窗③ K 段活→ssm 段活、窗⑤ V 的 258–347 死），与字节位置无关。

**"这样做是不是浪费了？"——分两层，答案都结论明确**：

1. **排布层：零浪费**。三区恰好铺满整根张量（对账式 6），没有任何字节被用于
   "排布本身"的 padding；K 与 ssm 共用区B 反而是**省**的（§2.5.4 解说第 8 条：
   不混存则页宽 +49%、容量 -33%）。
2. **图上的"死格"与字节顺序无关**。死格来自**统一页记账制度**（一个块号在三区
   各占一格、任何块号可发给任何组），不是稠密排布造成的——即使改成按页交织
   [A|B|C]×nb，同样这些字节照样死，且还额外牺牲连续性（违反 NOTE(zxr) 的 Ascend
   约束）。换言之：浪费的唯一来源是"等宽记账"，稠密字节序是**在既定制度下最优**
   （零拷贝、算子友好、铺满无 pad）的排布。
...
#### 2.5.9 从比特到槽：原生 V 与 OSCAR V 的存储入门（演讲用逐层引入）

**第 0 层 · 比特与字节**：bit = 一个 0/1 开关；Byte = 8 bit = **内存最小刻度
（买不了半个字节）**。类型 = 一个值花几个 bit：bf16 = 16bit（2B/值，浮点）；
int8 = 8bit（1B/值，256 档）；int2 = 2bit（**0.25B/值，只有 0/1/2/3 四档**）。

**第 1 层 · 一个 token 的 V 是什么**：每层每 KV 头产出一个 V 向量 = **256 个 bf16
数**；缓存的工作就是把它存下来供后续 token 的注意力使用。

**第 2 层 · 原生存法 = 照片（原值直存）**：256 个 bf16 × 2B = **512B 一字排开，
无任何加工**；写 = reshape_and_cache 拷贝，读 = 算子直接吃。V 槽宽度 512B 即由此
而来（K+V = 1,024B/token·head）。

**第 3 层 · OSCAR 存法 = 编号 + 图例**：
① 量化：对 256 个数各取 4 档编号 `idx = round((v−zero)/scale)`（例 zero=−1.0、
scale=1.0：v=1.7→编号 3，v=−0.9→编号 0）；② 图例 = V scale + V zero（fp16 各 2B
共 4B，住在 K 槽 +4..+8）；③ 打包：**4 个编号拼 1 字节**（int2 的 0.25B 由此落地，
`byte = idx₀|idx₁<<2|idx₂<<4|idx₃<<6`，取回 `(byte>>2k)&3`）→ 256 维 ÷ 4 =
**64B**。读回重建 `v ≈ idx×scale + zero`（每维仅 4 种取值）。

**第 4 层 · 账本**：原生 512B/向量 vs OSCAR 64B 编号 + 4B 图例 = **68B ≈ 7.5 压缩**，
代价是连续值变 4 档阶梯。

**第 5 层 · 装进格子**（= §2.5.5 四代阶梯表）：64B 编号写在 V 格子最前面——
legacy 几何 512B 格只写 64B（**假压缩，0 节省**：引擎按槽宽 512B/token 记账，
不知道你省了）；packed×2 现状 256B 格写 64B（**2× 真节省**：槽宽真的变小、
记账员跟着改）；PR 紧凑布局 68B 连排无格子（7.5×，需源码 PR）。

**第 6 层 · 误差三件套**（为什么 4 档也敢用）：① **旋转**——存前乘正交矩阵把
能量摊平各维（不平摊则离群维毁掉其余维）；② **裁剪**——按分位数（K 0.96/V 0.92）
砍离群让 min/max 收紧；③ **sink/recent 窗口**——头尾关键 token 另存 BF16 副本
（staging 仓库，见 §2.5.4 解说 3/5），只让中间长尾吃 4 档误差。

槽内字节级布局（packed×2，每 token·head）：

```text
区B 的 K 槽（256B）：
  [+0..+8)   元数据：K scale/zero ∥ V scale/zero（各 fp16 2B，小端）
  [+8..+32)  pad 24B（idx 区凑 32B 对齐）
  [+32..+96) K 编号 64B（256 维 ÷ 4 值/字节）
  [+96..+256) 闲置 160B（group16 预留）
区C 的 V 槽（256B）：
  [+0..+64)  V 编号 64B
  [+64..+256) 闲置 192B（group16 预留）
逻辑槽 160B = K 槽前 96B ⊕ V 槽前 64B（字节序 = N-01 契约，format.py:25-33）
```
