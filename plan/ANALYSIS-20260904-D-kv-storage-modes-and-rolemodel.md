...
区A（GDN conv 格，nb×15,360B → 字节 [0, 21,027,840)，宽 20.05MiB）
  ┌────┬────┬─…─┬────┬────┬────┬────┬─…─┬──────────────┐
  │死 0│死 1│ … │死42│G43 │G44 │G45 │ … │ 空(至块1368)   │ ← 块0-42=FULL 领走 → 区A 格死置
  └────┴────┴─…─┴────┴────┴────┴────┴─…─┴──────────────┘   块43-57=GDN(15块) → 装本层 conv

区B（K/ssm 双视图共区 → 字节 [21,027,840, 559,340,544)，宽 513.38MiB；一格=6 个 65,536B 逻辑块）
  ┌─ K[0..5] ─┬─ K[6..11] ─┬─…─┬─ K[252..257] ─┬─ ssm[43] ─┬─…─┬─ ssm[57] ─┬─────────────┐
  │ ←物理页0    │ ←物理页1    │ … │ ←物理页42      │ ←GDN状态   │ … │ ←GDN状态   │ 空(至块1368) │
  └────────────┴────────────┴─…─┴────────────────┴───────────┴─…─┴───────────┴─────────────┘
   K 视图 (1,369×6,128,1,256)bf16：该请求占 43×6=258 逻辑块 = 258×65,536B = 16.9MB
   ssm 视图 (1,369,12,128,128)bf16：该请求占 15 格 = 15×393,216B = 5.9MB

区C（V 格，FULL 专用 → 字节 [559,340,544, 1,097,653,248)，宽 513.38MiB；结构与区B 同）
  ┌─ V[0..5] ─┬─…─┬─ V[252..257] ─┬─ 死43 ─┬─…─┬─ 死57 ─┬─────────────┐
  │ ←物理页0   │ … │ ←物理页42      │ ←GDN块死置│ … │        │ 空(至块1368) │
  └───────────┴─…─┴────────────────┴────────┴─…─┴────────┴─────────────┘
   V 视图 (1,369×6,128,1,256)bf16：258 逻辑块 = 16.9MB

该请求在本池占用 = 43×786,432 + 15×408,576 = 39,945,216B ≈ 38.1MiB（58/1,369 = 4.24%）
全卡 16 池合计 ≈ 609MiB 有效字节（同批块号在 16 池各占一页 → 64 层全覆盖）
```
