...
区B  K/ssm 共区 [21,027,840, 559,340,544)  每格 393,216B
  ...
  读数（shape=容器总格数×每格形状；占用=领走的格数×每格字节）：
    K  视图 (8,214=1,369×6, 128, 1, 256)bf16：每逻辑块=128×1×256×2B=65,536B
        → 容器 8,214×65,536B=538,312,704B（整个区B）；本请求填 43 格×6=258 块=16.9MB
    ssm 视图 (1,369, 12, 128, 128)bf16：每格=12×128×128×2B=393,216B（=一份完整状态）
        → 容器 1,369×393,216B=538,312,704B（同一片字节，换尺子）；本请求 3 个 GDN
          租户层×各领 5 格=15 格=5.9MB（"3 层"来自池的三层合租，不在 shape 里）
