...
**格子归谁用？——块号**：引擎的分配单位是"块号"，由全局唯一 BlockPool 发牌、引用
计数互斥；一个块号 b 在三个区各占一格，归属决定格子的身份：
...
也不表示 GDN 占用从 0 号格开始——**占格子**层由发牌决定：块号 0–42 给 FULL、
43–57 给 GDN、58–1368 未发出（三个区的 58–1368 格同为空，与切法无关）。"混"的
只有 ssm≡K。）
...
nb 由预算单方面决定，与请求大小无关——请求只决定"领走几个块号"（§2.4）。也正因为
