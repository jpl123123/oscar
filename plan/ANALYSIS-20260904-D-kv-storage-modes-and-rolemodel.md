...
**格级对比图（审计修订版）：同一个 K 格（393,216B）· 原生 vs OSCAR packed×2**
（▓=信息字节 ░=对齐垫 空=闲置；数字经三重审计：format.py 常量 + store_kernel 写偏移
+ 独立复算/真机日志，审计表见下）

```text
━━ 原生 bf16：一格 768 个 token，每 token 512B 全是信息 ━━━━━━━━━━━━━━━
字节 0          512          1,024  …                          393,216
    ...
    │ 256 个 bf16  │             │     │                     │             │
    └─────────────┴─────────────┴─────┴─────────────────────┴─────────────┘
    768 × 512B = 393,216B ✓ · 信息率 100% · 每 token·head K+V = 1,024B

━━ OSCAR packed×2：同一格 1,536 个 token，每 token 256B（写 96B 跨度=72 信息+24 垫）━━
字节 0        256        512  …                                      393,216
    ...
    跨度写入 147,456B（37.5%）· 其中信息 110,592B（28.1%）

━━ token 槽内放大（审计修订：96B 跨度 = 72B 信息 + 24B 垫）━━
原生 K 槽（512B）                OSCAR K 槽（256B）
┌────────────────┐            ┌────────┬────────┬─────────┬───────────────┐
│ 256 个 bf16 原值 │            │ 8B 元数据│ 24B 垫  │ 64B K 编号│ 160B 闲置       │
│▓▓▓ 信息 100% ▓▓▓│            │▓▓▓▓▓▓▓▓│░░░░░░░░│▓▓▓▓▓▓▓▓│（group16 预留）  │
└────────────────┘            └─┬──────┴────────┴─────────┴───────────────┘
                                └── 写入跨度 96B ──┘（K_IDX_OFF=32 对齐用垫）

原生 V 槽（512B）                OSCAR V 槽（256B）
┌────────────────┐            ┌─────────────┬─────────────────────────┐
│ 256 个 bf16 原值 │            │ 64B V 编号   │ 192B 闲置（group16 预留）  │
└────────────────┘            └─────────────┴─────────────────────────┘
```

| 口径（每 token·head，K+V 合计） | 原生 bf16 | OSCAR packed×2 |
|---|---|---|
| 引擎记账宽度 | 1,024B | **512B**（×2 密度的来源） |
| OSCAR 写入跨度 | —（=记账宽） | 160B（K 96 + V 64） |
| 其中**信息** | 1,024B | **136B**（8 元数据 + 64 K 编号 + 64 V 编号） |
| 垫 + 闲置 | 0 | 24B 垫 + 352B 闲置 |
