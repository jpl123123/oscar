...
区B  K/ssm 共区 [21,027,840, 559,340,544)  每格 393,216B
  ...
  同一片区B（538,312,704B）的两种视图——按"块号格"统一表达，行行对账：

  |                    | K 视图（FULL 的尺子）                    | ssm 视图（GDN 的尺子）          |
  |--------------------|-----------------------------------------|--------------------------------|
  | shape（按块号格分解）| (**1,369**, **6**, 128, 1, 256) bf16 ¹    | (**1,369**, **12**, 128, 128) bf16 |
  | 每块号格元素数       | 6×128×1×256 = **196,608**                | 12×128×128 = **196,608** ✓ 相等  |
  | 每块号格字节         | 196,608 × 2B = **393,216B**              | 196,608 × 2B = **393,216B** ✓ 相等 |
  | 容器总账            | 1,369 × 393,216B = **538,312,704B**      | 1,369 × 393,216B = **538,312,704B** ✓ |
  | 格内细分            | 每格再切 6 个逻辑块 × 65,536B（=128×1×256×2B） | 不细分（一份完整 DeltaNet 状态）   |
  | 本请求占用          | 43 格 × 6 = **258 逻辑块** = 16.9MB       | 3 租户层 × 5 格 = **15 格** = 5.9MB |

  ¹ K 视图的实现 shape 把前两维展平：(1,369×6, 128, 1, 256) = (8,214, 128, 1, 256)；
  逻辑块号 L = 块号×6 + i 正是"先按块号分组、再取组内第 i 个"的展平。
  两视图的**元素数对称**（6×128×1×256 = 12×128×128 = 196,608）就是 assert
  512×768==393,216 的 shape 级写法：每块号格装同样多的 bf16，只是排法不同
  （FULL 排成 6×128 个 token 行、GDN 排成 12 个 128×128 矩阵）。
  （"3 租户层" = 池内合租的组0/1/2 各第 i 层，各领各的块号，故不在 shape 里）
